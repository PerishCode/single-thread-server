...
impl From<String> for HttpRequest {
    fn from(request: String) -> Self {
        let mut parsed_method = Method::Uninitilized;
...
fn process_header_line(s: &str) -> (String, String) {
    let mut header_pair = s.split(":");
    let mut key = String::from("");
    let mut value = String::from("");

    if let Some(k) = header_pair.next() {
        key = k.to_string();
    }

    if let Some(v) = header_pair.next() {
        value = v.to_string();
    }
...
mod tests {
    ...
    fn test_read_http() {
        ...
        assert_eq!(Resource::Path("/greeting".into()), request.resource);
